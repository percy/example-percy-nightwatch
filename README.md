...
by opening the
[`index.html`](https://github.com/percy/example-percy-nightwatch/blob/master/index.html)
file in a browser.
...
This will run the app's Nightwatch tests, which contain calls to create Percy snapshots. The snapshots
...
The repository uses the following key dependency versions (updated):
...
These were updated to ensure compatibility with the local browser and Percy CLI.
